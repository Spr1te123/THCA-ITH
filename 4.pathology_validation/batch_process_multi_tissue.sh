...
while IFS= read -r FILE; do
    ...
    python wsi_processing_multi_tissue.py \
        ...
        --output "/data/tiatoolbox/results" \
        --single-wsi "$FULL_PATH" \
        > "/data/tiatoolbox/results/log_${BASENAME}_gpu${GPU_ID}.txt" 2>&1
    
    if [ $? -eq 0 ]; then
        echo "✓ Success: $BASENAME"
        # Check if multiple tissue regions were processed
        if grep -q "Found [2-9]\|[0-9][0-9]* tissue regions" "/data/tiatoolbox/results/log_${BASENAME}_gpu${GPU_ID}.txt"; then
            REGIONS=$(grep -o "Found [0-9]* tissue regions" "/data/tiatoolbox/results/log_${BASENAME}_gpu${GPU_ID}.txt" | head -1)
            echo "  Note: $REGIONS"
        fi
    else
        echo "✗ Failed: $BASENAME (check log)"
        # Show last error lines
        tail -n 5 "/data/tiatoolbox/results/log_${BASENAME}_gpu${GPU_ID}.txt"
    fi
done < "$FILES_LIST"
