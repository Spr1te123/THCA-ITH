...
1. **Memory Error**
   - Reduce number of Optuna trials (`N_TRIALS`)
   - Decrease CV folds (`N_FOLDS`)
   - Process feature combinations sequentially

2. **Poor External Validation**
   - Check data distribution differences
   - Review feature preprocessing consistency
   - Consider domain adaptation techniques
   - Verify no data leakage

3. **Low Sensitivity**
   - Adjust screening threshold lower (e.g., 0.15)
   - Increase `min_sensitivity` constraint
   - Try different resampling methods
   - Check class imbalance ratio

4. **Convergence Warnings (LightGBM)**
   - Increase `n_estimators`
   - Adjust `learning_rate` (lower)
   - Modify `num_leaves` and `max_depth`

5. **SHAP Computation Slow**
...
- Verify consistent preprocessing across train/validation
- Check for data leakage (especially in feature engineering)
- Balance batch effects if using multi-center data

### Model Training
- Always use stratified splits for imbalanced data
- Validate on external cohort if available
- Save intermediate results (models, predictions)
- Document all hyperparameters and settings

### Evaluation
- Report 95% confidence intervals (bootstrap)
- Use appropriate metrics for imbalanced data
- Perform sensitivity analysis on thresholds
- Compare to clinical baseline models

### Deployment Considerations
- Calibrate probabilities before clinical use
- Set threshold based on clinical consequences
...
- **v1.0** (2024): Initial release with LightGBM, Optuna, SHAP integration
...
- Imbalanced-learn: Resampling techniques for imbalanced data
...
- Deep learning integration (TabNet, Neural ODEs)
