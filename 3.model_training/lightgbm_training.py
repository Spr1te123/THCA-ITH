...
# 加载所有数据
try:
    clinical_df = read_csv_with_encoding('F:\\Habitat_radiomics\\clinical_features_processed_zlyy.csv')
    radiomics_df = read_csv_with_encoding('F:\\Habitat_radiomics\\radiomics_features_zlyy.csv')
    iTED_df = read_csv_with_encoding('F:\\Habitat_radiomics\\iTED_features_zlyy.csv')
    ITHscore_df = read_csv_with_encoding('F:\\Habitat_radiomics\\3D_ITHscore_zlyy.csv')
except Exception as e:
...
try:
    external_clinical_df = read_csv_with_encoding('F:\\Habitat_radiomics\\clinical_features_processed_ydyy.csv')
    external_radiomics_df = read_csv_with_encoding('F:\\Habitat_radiomics\\radiomics_features_ydyy.csv')
    external_iTED_df = read_csv_with_encoding('F:\\Habitat_radiomics\\iTED_features_ydyy.csv')
    external_ITHscore_df = read_csv_with_encoding('F:\\Habitat_radiomics\\3D_ITHscore_ydyy.csv')
...
report += f"""
    ...
    6. 临床应用建议
    ------------------------------------------
1. 考虑到M1（有转移）的低流行率（13.6%），建议使用推荐的决策阈值而非默认的0.5
2. 本研究使用了概率校准技术，改善了模型的校准性能
3. 采用了改进的模型选择策略，避免选择极端不平衡的模型
4. 在临床决策中，应权衡假阴性（漏诊转移）和假阳性（过度治疗）的成本
5. 建议定期使用新数据验证和更新模型
6. 考虑收集更多M1病例以改善类别平衡
"""
...
risk_report += f"""
推荐使用: Risk_Group_Clinical
原因: 基于筛查优化阈值 ({best_threshold:.2f})，专为高敏感性筛查工具设计，更符合临床筛查需求

输出文件说明:
1. all_patients_predictions_{best_model_name}_zlyy.csv
  - 包含所有患者的完整预测信息和多种风险分组方法

2. patients_risk_groups_for_protein_analysis_{best_model_name}.csv
  - 简化版本，专门用于蛋白表达分析
  - 包含: PatientID, Predicted_Probability, Risk_Group_Tertile, Risk_Group_Clinical, True_M_stage

下一步分析建议:
1. 使用风险分组结果与蛋白表达数据进行关联
2. 比较不同风险组之间的蛋白表达差异（如ANOVA、Kruskal-Wallis检验）
3. 识别与风险分组相关的关键蛋白标志物
4. 考虑构建蛋白-影像组学联合模型
"""
