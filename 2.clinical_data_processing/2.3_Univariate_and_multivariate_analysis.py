...
# 设置输出目录
OUTPUT_DIR = '/results/Publication_Results'
...
def load_complete_data(dataset_type='zlyy'):
    ...
    # 加载临床数据
    clinical_file = f'/data/clinical_features_processed_{dataset_type}.csv'
    clinical_data = pd.read_csv(clinical_file)

    # 加载iTED特征
    ited_file = f'/data/iTED_features_{dataset_type}.csv'
    ited_features = pd.read_csv(ited_file)

    # 加载Radiomics特征
    radiomics_file = f'/data/radiomics_features_{dataset_type}.csv'
    radiomics_features = pd.read_csv(radiomics_file)

    # 加载3D_ITHscore
    ithscore_file = f'/data/3D_ITHscore_{dataset_type}.csv'
...
def create_risk_groups_from_predictions(data, dataset_type, feature_type):
    ...
    if feature_type == 'iTED':
        pred_file = f'/data/iTED_predictions_all_patients_{dataset_type}.csv'
    elif feature_type == 'Radiomics':
        pred_file = f'/data/Radiomics_predictions_all_patients_{dataset_type}.csv'
    elif feature_type == '3D_ITH':
        # 3D_ITHscore直接使用原始值
        pred_file = f'/data/3D_ITHscore_{dataset_type}.csv'
    else:
