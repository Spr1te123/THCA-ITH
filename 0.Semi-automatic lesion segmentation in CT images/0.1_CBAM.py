...
class CBAMPlainConvUNet(nn.Module):
    def __init__(self,
        ...
        self.decoder = CBAMUNetDecoder(self.encoder, num_classes, n_conv_per_stage_decoder, deep_supervision,
                                   nonlin_first=nonlin_first)
        print('using cbam unet...')
    ...
    def __init__(self,
        ...
        for s in range(n_stages):
            ...
            if pool == 'max' or pool == 'avg':
                ...
                raise RuntimeError()
            stage_modules.append(CBAMStackedConvBlocks(
...
class CBAMUNetDecoder(nn.Module):
    def __init__(self,
                 encoder: Union[CBAMPlainConvEncoder],
                 num_classes: int,
        ...
        for s in range(1, n_stages_encoder):
            ...
            stages.append(CBAMStackedConvBlocks(
...
class CBAMStackedConvBlocks(nn.Module):
    def __init__(self,
                 num_convs: int,
                 conv_op: Type[_ConvNd],
                 input_channels: int,
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.sa(x)  # Spatial Attention on channel-attended features
        return x
